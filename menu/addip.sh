...
adminip=$( curl -sS https://raw.githubusercontent.com/pcfreak/allow/main/accessmenu | grep $MYIP )
...
sleep 1
cek=$( curl -sS https://raw.githubusercontent.com/pcfreak/allow/main/ipvps.conf | awk '{print $5}' | grep $daftar )
if [[ $daftar = $cek ]]; then
...
rm -rf /root/allow
git config --global user.email "warrenpretorius8@gmail.com"
git config --global user.name "pcfreak"
git clone https://github.com/pcfreak/allow.git
mkdir /root/allow
cd /root/allow/
rm -rf .git
git init
...
git branch -M main
git remote add origin https://github.com/pcfreak/allow.git
git push -f https://${tokengit}@github.com/pcfreak/allow.git
